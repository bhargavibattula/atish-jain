...
import { BookOpen, Award, Code2, Users, ArrowRight, TrendingUp, Zap, Star, LogOut, Clock, Lock, ShieldAlert } from "lucide-react";
...
export default function StudentDashboard({ user, session }: StudentDashboardProps) {
  ...
  const isApproved = user?.membershipStatus === "approved" && user?.membership;

  const stats = [
    {
      label: "Courses Enrolled",
      value: isApproved ? (user?.progress?.length || 0) : 0,
      icon: BookOpen,
      color: "text-blue-400",
      bg: "bg-blue-500/15",
    ...
    },
    {
      ...
      value: isApproved ? (user?.certificates?.length || 0) : 0,
      icon: Award,
  ...
  const handleSidebarClick = (e: React.MouseEvent, href: string) => {
    if (!isApproved && href !== "/student") {
      e.preventDefault();
      toast.error("Please apply for a membership or wait for admin approval to unlock this section.");
    }
  };
    ...
    <div className="min-h-screen bg-[#0B0F19]">
      ...
      <div className="flex">
          ...
          <nav className="space-y-1 flex-1">
            {[
              ...
              <Link
                key={item.href}
                href={item.href}
                onClick={(e) => handleSidebarClick(e, item.href)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 ${
                  item.active
                    ? "bg-blue-500/15 text-blue-400 border border-blue-500/20"
                    : !isApproved && item.href !== "/student"
        ...
        <main className="flex-1 lg:ml-64 p-6 lg:p-8">
          ...
          {/* Conditional Rendering of Dashboard Content */}
          {user?.membershipStatus === "pending" ? (
            ...
            <motion.div
              ...
              className="max-w-2xl mx-auto mt-8 p-8 rounded-3xl bg-[#111827]/40 border border-amber-500/20 backdrop-blur-xl shadow-2xl relative overflow-hidden text-center"
            >
              <div className="absolute top-0 right-0 w-[200px] h-[200px] rounded-full bg-amber-500/5 blur-[80px] pointer-events-none" />
              
              <div className="w-20 h-20 bg-amber-500/10 border border-amber-500/30 rounded-full flex items-center justify-center mx-auto mb-6 relative">
                <span className="absolute inset-0 rounded-full border border-amber-500/30 animate-ping opacity-75" />
                <Clock className="text-amber-400 w-10 h-10 animate-pulse" />
              </div>
              
              <h2 className="font-poppins font-bold text-3xl text-white mb-3">Application Under Review</h2>
              <p className="text-gray-300 text-sm leading-relaxed mb-6 max-w-lg mx-auto">
                Hi <span className="text-white font-semibold">{firstName}</span>, your request to join the <span className="text-amber-400 font-semibold capitalize">{user?.membershipTypeRequested || "Diamond"} Plan</span> is currently pending approval.
              </p>
              
              <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.04] text-left mb-6 space-y-4">
                <div className="flex items-center justify-between text-xs border-b border-white/5 pb-2.5">
                  <span className="text-gray-500 font-medium">WhatsApp Number</span>
                  <span className="text-white font-semibold">{user?.whatsappNumber || "Not Provided"}</span>
                </div>
                <div className="flex items-center justify-between text-xs border-b border-white/5 pb-2.5">
                  <span className="text-gray-500 font-medium">Admission Status</span>
                  <span className="text-amber-400 font-bold uppercase tracking-wider animate-pulse">Pending Admin Approval</span>
                </div>
                {user?.applicationMessage && (
                  <div className="text-xs">
                    <span className="text-gray-500 font-medium block mb-1">Your Statement:</span>
                    <p className="text-gray-400 bg-black/20 p-3 rounded-xl border border-white/5 leading-relaxed italic">
                      "{user.applicationMessage}"
                    </p>
                  </div>
                )}
              ...
              <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10 text-blue-400 text-xs leading-relaxed max-w-lg mx-auto">
                Once the administrator approves your profile, your entire course curriculum and coding sandbox will instantly unlock on this page.
              </div>

              <button
                onClick={() => router.refresh()}
                className="mt-6 inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 text-white font-semibold text-sm transition-all"
              >
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.5 }}
              className="max-w-2xl mx-auto mt-8 p-8 rounded-3xl bg-[#111827]/40 border border-red-500/20 backdrop-blur-xl shadow-2xl relative overflow-hidden text-center"
            >
              ...
              <h2 className="font-poppins font-bold text-3xl text-white mb-3">Application Declined</h2>
              <p className="text-gray-300 text-sm leading-relaxed mb-6 max-w-lg mx-auto">
                Unfortunately, your application for the membership plan was declined. Please verify your details or apply for another tier.
              </p>
              
              <Link
                href="/memberships"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl btn-gradient text-white font-semibold text-sm transition-all"
              >
                Browse Membership Plans <ArrowRight size={14} />
              </Link>
            </motion.div>
          ) : !isApproved ? (
            /* NO ACTIVE MEMBERSHIP (NONE / UNLOCKED) VIEW */
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.5 }}
              className="max-w-2xl mx-auto mt-8 p-8 rounded-3xl bg-[#111827]/40 border border-white/10 backdrop-blur-xl shadow-2xl relative overflow-hidden text-center"
            >
              <div className="absolute top-0 right-0 w-[200px] h-[200px] rounded-full bg-blue-500/5 blur-[80px] pointer-events-none" />
              ...
              <h2 className="font-poppins font-bold text-3xl text-white mb-3">Unlock Your Curriculum</h2>
              <p className="text-gray-300 text-sm leading-relaxed mb-6 max-w-lg mx-auto">
                You do not have an active membership. To start building production-ready AI apps, browse and apply for one of our training tracks.
              </p>
              
              <Link
                href="/memberships"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl btn-gradient text-white font-semibold text-sm transition-all animate-pulse"
              >
                Browse Membership Plans <ArrowRight size={14} />
              </Link>
            </motion.div>
          ) : (
            /* APPROVED / ACCESSIBLE MEMBERSHIP VIEW */
            <>
              {/* Stats */}
                ...
                >
                  ...
                  {user?.certificates?.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600 text-xs mt-1 leading-relaxed">Complete curriculum modules to unlock certificates.</p>
                    </div>
                  )}
                </motion.div>
              </div>
